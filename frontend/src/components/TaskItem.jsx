/**
 * components/TaskItem.jsx — V2
 * Tous les contrôles toujours visibles (application consultant/admin uniquement).
 */
    ...
    <div style={{
        ...
        {editing ? (
          <div style={{ flex:1, display:'grid', gap:'0.5rem' }}>
            ...
            <textarea rows={2} value={editDesc} onChange={e => setEditDesc(e.target.value)}
              placeholder="Description (optional)" style={{ resize:'vertical', fontSize:'0.83rem' }} />
          ...
          <>
            ...
            <div style={{ display:'flex', gap:'0.25rem', flexShrink:0 }}>
              ...
              <button className="btn-icon" title="Delete" onClick={deleteTask} style={{ color:'#f87171' }}>✕</button>
            </div>
          </>
        )}
      </div>
      ...
      {expanded && !editing && (
        <div style={{ padding:'0 1rem 1rem', borderTop:'1px solid #2e2e33' }}>
          {task.description && (
            <p style={{ fontSize:'0.83rem', marginTop:'0.75rem', marginBottom:'0.75rem' }}>{task.description}</p>
          )}
